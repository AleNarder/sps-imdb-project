...
<tsung loglevel="info">
    ...
    <sessions>
        <session probability="100" name="pgsql_testing" type="ts_pgsql">

            <transaction name="connection">
                <request>
                    <pgsql type="connect" database="imdb" username="authenticator" />
                </request>
            </transaction>

            <!-- DOES NOT WORK
            <request><pgsql type="authenticate" password="sps"/></request>
            -->
            ...
            <for from="1" to="{ args.userequests }" var="i">
                ...
                <transaction name="query">
                    <!-- Execute Request -->
                    <request subst="true"><pgsql type="sql">SELECT * from imdb.title_details where tconst='%%_id%%';</pgsql></request>
                </transaction>
